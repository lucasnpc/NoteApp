...
class NotesViewModel @Inject constructor(private val noteUseCases: NoteUseCases) : ViewModel() {
    ...
    fun onEvent(event: NotesEvent) {
        when (event) {
            is NotesEvent.Order -> {
                if (verifyStateNEventEquality(event))
                    return
    ...
    private fun verifyStateNEventEquality(event: NotesEvent.Order): Boolean =
        state.value.noteOrder::class == event.noteOrder::class && state.value.noteOrder.orderType ==
